...
<template>
  <main class="login-page">
    <section class="login-visual">
      ...
      <div class="login-message"><p class="eyebrow light">SCENIC MOBILITY OPERATIONS</p><h1>让每一辆车<br />都在视野之内。</h1><p>地图、订单、财务和系统状态，在一个安静而清晰的运营工作台中协同。</p></div>
      <div class="login-signal"><span><i class="pulse" />车辆在线监测</span><span>多景区运营</span><span>安全审计</span></div>
    </section>
    <section class="login-panel">
      <div class="login-form-wrap">
        <p class="eyebrow">ADMIN CONSOLE</p><h2>欢迎回来</h2><p class="login-subtitle">使用超级管理员账号登录运营管理台</p>
        <el-alert v-if="route.query.expired === '1'" title="会话已过期" description="为保护管理数据，请重新登录。" type="info" :closable="false" show-icon />
        <el-alert v-if="isMockMode" title="当前为 Mock 交互原型" description="账号填 admin，密码输入任意 6 位以上内容。不会连接生产数据。" type="warning" :closable="false" show-icon />
  ...
  <el-dialog v-model="forcePasswordOpen" title="首次登录必须修改密码" width="520px" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-alert title="初始化密码仅可用于首次登录" description="新密码至少 12 位，并同时包含大写字母、小写字母和数字。修改完成前无法访问业务数据。" type="warning" :closable="false" show-icon />
