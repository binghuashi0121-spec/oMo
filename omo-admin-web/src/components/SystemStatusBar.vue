...
const props = defineProps<{ health: SystemHealth | null }>();
defineEmits<{ refresh: [] }>();
...
<template>
  <div class="system-bar" :class="`is-${health?.level || 'loading'}`">
    <div class="system-summary"><AppIcon :name="icon" /><strong>{{ health ? label : '状态检查中' }}</strong><span>{{ health?.summary || '正在聚合各服务健康状态…' }}</span></div>
    <div class="system-meta" v-if="health"><span>数据更新于 {{ relativeFreshness(health.dataFreshnessAt) }}</span><router-link to="/system">查看诊断</router-link><el-button text circle aria-label="刷新系统状态" @click="$emit('refresh')"><AppIcon name="refresh" /></el-button></div>
  </div>
</template>
