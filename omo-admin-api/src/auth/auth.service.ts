...
export const passwordHashOptions = { type: argon2.argon2id, memoryCost: 19_456, timeCost: 2, parallelism: 1 } as const;
...
export class AuthService {
  ...
  async login(usernameRaw: string, password: string, ip: string, userAgent: string, requestId: string) {
    ...
    const valid = user ? await argon2.verify(user.passwordHash, password) : (await argon2.hash(password, passwordHashOptions), false);
  ...
  async changePassword(user: AdminUserRecord, session: AdminSessionRecord, currentPassword: string, newPassword: string, requestId: string) { if (!(await argon2.verify(user.passwordHash, currentPassword))) throw new ForbiddenException('当前密码不正确'); if (currentPassword === newPassword) throw new ForbiddenException('新密码不能与当前密码相同'); const passwordHash = await argon2.hash(newPassword, passwordHashOptions); await this.repository.updateAdmin(user.id, { passwordHash, mustChangePassword: false, updatedAt: new Date().toISOString() }); user.passwordHash = passwordHash; user.mustChangePassword = false; await this.audit(user.id, 'auth.password_changed', 'admin_user', user.id, requestId, { sessionId: session.id }); }
